    ...
    if selected =="Predict Diabetes":
        ...
        if st.button('Predict'):
            ...
            def generate_report(Name, prediction):
                ...
                c.drawString(100, 730, f"No. Of pregnancies: {Pregnancies}")
                c.drawString(100, 730, f"Plasma glucose concentration at 2 Hours in an oral glucose tolerance test (GTIT): {Glucose}")
                c.drawString(100, 730, f"DiaStolic Blood Pressure (mm Hg) : {BloodPressure}")
                c.drawString(100, 730, f"Triceps skin fold Thickness (mm) : {SkinThickness}")
                c.drawString(100, 730, f"2-Hour Serum insulin (µh/ml)  : {Insulin}")
                c.drawString(100, 730, f"BMI: {BMI}")
                c.drawString(100, 730, f"Diabetes pedigree function : {DiabetesPedigreeFunction}")
                c.drawString(100, 730, f"Age(years)  : {Age}")
